...
func TestFromEnvUsesDefaults(t *testing.T) {
	...
	// Unset leaves this at 0; auth and server apply the default minimum.
	if cfg.PasswordMinLength != 0 {
		t.Fatalf("PasswordMinLength = %d, want 0 when unset", cfg.PasswordMinLength)
	}
...
func TestFromEnvWithProcessOverridesEnvironmentProcess(t *testing.T) {
	...
	cfg, err := FromEnvWithProcess(ProcessWeb)
	if err != nil {
