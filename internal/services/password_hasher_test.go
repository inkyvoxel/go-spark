...
func TestArgon2idHasherHashAndVerifyWithPepper(t *testing.T) {
	hasher := newArgon2idHasher(AuthOptions{
		Argon2idMemoryKiB:   64,
		Argon2idIterations:  1,
		Argon2idParallelism: 1,
...
func TestArgon2idHasherVerifyFailsWithWrongPepper(t *testing.T) {
	hasher := newArgon2idHasher(AuthOptions{
		Argon2idMemoryKiB:   64,
		Argon2idIterations:  1,
		Argon2idParallelism: 1,
	...
	ok, err := otherHasher.Verify(hash, "correct horse battery staple")
	if err != nil {
		t.Fatalf("Verify() error = %v", err)
	}
	if ok {
...
func TestArgon2idHasherBlankPepperUsesUnpepperedBehavior(t *testing.T) {
	hasher := newArgon2idHasher(AuthOptions{
		Argon2idMemoryKiB:   64,
		Argon2idIterations:  1,
		Argon2idParallelism: 1,
