...
## 1. Fix AGENTS.md onboarding wall

`AGENTS.md` setup says `cp .env.example .env` → `make migrate-up` → `make start`,
but `make start` hard-fails because `validateSecurityConfig` requires
`SECRET_KEY_BASE`, `AUTH_TOTP_KEY`, and `AUTH_PASSWORD_PEPPER`, which are empty
in `.env.example`. `migrate-up` works (no validation); `start` dies. An agent
following AGENTS.md literally hits a dead end.

- Add the secret-generation step (or point to `make init`) in `AGENTS.md`.
- Files: `AGENTS.md`. Cross-check against `internal/app/build.go` `validateSecurityConfig`.

## 2. Ship an example non-auth feature (the "projects" CRUD)
...
## 3. Apply security headers to static + health endpoints
...
## 4. Add a maximum password length
...
## 5. Decide: hand-rolled OTP vs battle-tested library
...
## 6. Consolidate the signed-cookie helpers
...
## 7. Decide email-change session revocation policy
...
## 8. Harden init.sh substring + sed edge cases
...
## 10. Write the CHANGELOG before tagging v1
